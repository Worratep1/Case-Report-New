  ...
  const handleFileChange = (e) => {
    ...
    files.forEach((file) => {
      if (file.size > CONFIG.MAX_FILE_SIZE_BYTES) {
        errors.push(`${file.name}: ขนาดเกิน ${CONFIG.MAX_FILE_SIZE_MB}MB`);
      } else if (
    ...
    if (errors.length > 0)
      alert(`ไม่สามารถอัปโหลดบางไฟล์ได้:\n- ${errors.join("\n- ")}`);
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-5 text-left">
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase mb-1 flex items-center gap-1.5">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
              ...
              <div>
                ...
                <div
                  ...
                  className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-50 hover:border-indigo-400 transition-colors group"
                >
                  <div className="w-12 h-12 bg-slate-100 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-2 group-hover:bg-indigo-50 group-hover:text-indigo-500 transition-colors">
                    <FileText size={24} />
                  </div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Max {CONFIG.MAX_FILE_SIZE_MB}MB (PDF, JPG, PNG)
                  </p>
                  <input
                    type="file"
                    multiple
                ...
                {attachedFiles.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {attachedFiles.map((file, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm"
                      >

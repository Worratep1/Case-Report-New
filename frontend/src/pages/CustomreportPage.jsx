...
// ✅ 2.2 โหลด Cases ตามวันที่
  const fetchCases = async () => {
      // if (!selectedDate) return; // เอาออกก่อนเผื่อ date เป็นค่าว่าง
      setLoadingData(true);
      try {
          console.log("Fetching cases for:", selectedDate); // 🔍 Debug 1
          const res = await getCases({ date: selectedDate });
          
          const rawCases = res.cases || [];
          console.log("Raw cases:", rawCases); // 🔍 Debug 2

          // แปลงข้อมูลให้ตรงกับ UI
          const mappedCases = rawCases.map(c => {
              // ใช้ ?. กัน Error กรณี lookupData ยังไม่มา
              ...
              const problemObj = lookupData.problems.find(p => p.problem_id === c.problem_id); // ✅ ตรงนี้ต้อง problems มี s

              // ... (ส่วนคำนวณเวลา ใช้โค้ดเดิม) ...
  ...
  useEffect(() => {
      // ✅ ย้ายเงื่อนไข: ให้โหลดข้อมูลได้เลยแม้ status จะว่าง (เพื่อดูว่า Case มาไหม)
      // แต่เช็คว่า products หรือ statuses อันใดอันหนึ่งมาก็พอ
      if (lookupData.statuses.length > 0 || lookupData.products.length > 0) {
          fetchCases();
      }
  }, [selectedDate, lookupData]);
  // --- DASHBOARD DATA CALCULATION ---
  // Logic from DailyReport adapted for CustomReport
  ...
  const dashboardData = useMemo(() => {
      ...
      casesOfSelectedDate.forEach(c => {
          ...
          // Game Map for Bar Chart
          const game = c.game || "Unknown";
          gameMap[game] = (gameMap[game] || 0) + 1;
      });

      // Prepare Pie Data using STATUS_CONFIG
...
const openNewCaseModal = () => {
  ...
  const openEditModal = (item) => {
    // item คือข้อมูลที่ map มาแล้ว แต่เราต้องการค่า ID เดิม (raw_...) มาใส่ใน Form
    setCurrentCase({
        ...item,
...
// ✅ ฟังก์ชันเปิด Modal ลบ (ที่หายไป)
  ...
  const confirmSave = async () => {
   setIsLoading(true); // เปิด Loading ระหว่างบันทึก
    try {
        // 1. เตรียม Payload (ข้อมูลที่จะส่งไป Backend)
        // ต้องแปลงวันที่และเวลาให้เป็น format ที่ Backend ต้องการ (YYYY-MM-DDTHH:mm:00.000)
        ...
        const payload = {
            ...
            // created_by: 1 // ถ้ามีระบบ Login ให้ใส่ ID คน login
        };
        ...
        if (currentCase.id === null) {
            ...
            // --- กรณีแก้ไข (Update) ---
            await updateCase(currentCase.id, payload); // ใช้ ID ของเคส
            alert("แก้ไขข้อมูลสำเร็จ! ✅");
        }

        // 2. ปิด Modal และโหลดข้อมูลใหม่
        setIsSaveConfirmModalOpen(false);
        setIsEditModalOpen(false);
        fetchCases(); // 🔄 ดึงข้อมูลล่าสุดจาก DB
  ...
  // ✅ ฟังก์ชันลบข้อมูล (แก้ไขแล้ว)
  const confirmDelete = async () => {
    // 1. ตรวจสอบว่ามีข้อมูลที่จะลบหรือไม่
    if (!currentCase || !currentCase.id) return;

    try {
        // 2. เรียก API ลบข้อมูลที่ Backend
        ...
        // 5. โหลดข้อมูลใหม่จาก Database เพื่อให้ตารางอัปเดต
        fetchCases(); 
  ...
  const handleSendEmail = async () => {
    ...
    setIsLoading(true);
    // TODO: [BACKEND] ตรงนี้เรียก API ส่งอีเมล พร้อมแนบไฟล์
    // TODO: [BACKEND] นำไฟล์ใน attachedFiles ไปแนบใน request ของ API
    setTimeout(() => {
        setIsLoading(false);
...
// ✅ สร้างตัวเลือก Filter จาก DB
  const filterStatusOptions = useMemo(() => {
    ...
    const dbOpts = lookupData.statuses.map(s => ({
        value: s.status_name.toLowerCase().replace(/\s+/g, ''), // ทำให้ตรงกับ key
...
const modalGameOptions = useMemo(
  ...
  [lookupData.products]   // ✅ เปลี่ยนจาก products เป็น lookupData.products
);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex justify-between items-end">
            <div>
                <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                    Report on <span className="text-indigo-600 border-b-2 border-indigo-600/20 px-1">{selectedDate}</span>
                </h2>
                <p className="text-slate-500 text-sm mt-1">จัดการข้อมูล รายละเอียด และสถานะของเคส</p>
            </div>
        ...
        <div className="bg-white p-4 rounded-t-xl border border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-2 w-full sm:w-auto">
             ...
             {/* <span className="bg-slate-100 text-slate-600 text-xs px-2 py-0.5 rounded-full font-medium">{filterStatusOptions.length} รายการ</span> */}
          </div>
          ...
          <p>ไม่พบข้อมูล</p>
           ...
           <div className="bg-white px-6 py-4 border-t border-slate-200 flex items-center justify-between">
            <span className="text-sm text-slate-500">
              หน้า {currentPage} จาก {totalPages} ({filterStatusOptions.length} รายการ)
            </span>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] ">
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 ">
                    <h3 className="font-bold text-xl text-slate-800 flex items-center gap-2">
                        ...
                        {currentCase.id === null ? 'เพิ่มเคสใหม่' : `แก้ไขข้อมูลเคส #${currentCase.id}`}
                    </h3>
                ...
                <form onSubmit={handleInitiateSave} className="p-6 overflow-y-auto custom-scrollbar space-y-4 ">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-left">
                         <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1 ml-1">เกม</label>
                            <CustomSelect 
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1 text-left ml-1 ">สถานะ</label>
                            <CustomSelect 
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1 text-left ml-1">ปัญหา</label>
                        <CustomSelect 
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1 text-left ml-1">รายละเอียด</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1 text-left ml-1">วิธีการแก้ไข</label>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                    <button 
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 border rounded-xl bg-white text-left transition-all ${isRecipientDropdownOpen ? 'border-indigo-500 ring-4 ring-indigo-500/10 shadow-sm' : 'border-slate-200 hover:bg-slate-50'}`}
                    >
                        <div className="flex flex-wrap gap-2 items-center w-full overflow-hidden">
                             {selectedRecipientIds.length === 0 ? (
                                <span className="text-slate-400 text-sm">Select email addresses...</span>
                             ) : (
                                <>
                                    <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-xs font-bold whitespace-nowrap">{selectedRecipientIds.length} Selected</span>
                                    <span className="text-sm text-slate-600 truncate flex-1 ">
                                        {availableRecipients.filter(u => selectedRecipientIds.includes(u.recipient_id)).map(u => u.email).join(', ')}
                                    </span>
                                </>
                             )}
                        </div>
                     ...
                     {isRecipientDropdownOpen && (
                        <div className="absolute z-20 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl max-h-60 overflow-y-auto p-2 text-left animate-in fade-in zoom-in-95 duration-100">
                            {availableRecipients.length > 0 ? (
                                availableRecipients.map(user => (
                                    <div 
                                        key={user.recipient_id} 
                                        onClick={() => toggleRecipient(user.recipient_id)} 
                                        className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${selectedRecipientIds.includes(user.recipient_id) ? 'bg-indigo-50 text-indigo-700' : 'hover:bg-slate-50 text-slate-700'}`}
                                    >
                                        <div className={`shrink-0 ${selectedRecipientIds.includes(user.recipient_id) ? 'text-indigo-600' : 'text-slate-300'}`}>
                                            {selectedRecipientIds.includes(user.recipient_id) ? <CheckSquare size={20}/> : <Square size={20}/>}
                                        </div>
                                        <div className="flex flex-col min-w-0">
                                            <span className="text-sm truncate text-slate-500 font-medium text-left">{user.name}</span>
                                            <span className="font-bold text-sm truncate text-slate-800 text-left ">{user.email}</span>
                                        </div>
                                    </div>
                                ))
                            ) : (
                                <div className="p-4 text-center text-slate-400 text-sm">No recipients found</div>
                            )}
                        </div>
                    )}
                </div>
              ...
              <div className="space-y-3">
                <div>
                   <label className="block text-xs font-bold text-slate-500 uppercase mb-1 ml-1 text-left">Subject</label>
                   <input 
                    value={emailSubject} 
                ...
                <div>
                   <label className="block text-xs font-bold text-slate-500 uppercase mb-1 ml-1 text-left">Message</label>
                   <textarea 
                    value={emailBody} 
                    onChange={(e) => setEmailBody(e.target.value)} 
                    rows={4} 
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" 
                 />
                </div>
              </div>
              ...
              <div>
                ...
                <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-50 hover:border-indigo-400 transition-colors group">
                  <div className="w-12 h-12 bg-slate-100 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-2 group-hover:bg-indigo-50 group-hover:text-indigo-500 transition-colors"><FileText size={24} /></div>
                  <p className="text-sm text-slate-600 font-medium">Click to upload documents</p>
                  <p className="text-xs text-slate-400 mt-1">Max {CONFIG.MAX_FILE_SIZE_MB}MB (PDF, JPG, PNG)</p>
                  <input type="file" multiple className="hidden" ref={fileInputRef} onChange={handleFileChange} accept={CONFIG.ALLOWED_FILE_TYPES.join(',')} />
                </div>
                {/* File List */}
                  ...
                  <div className="mt-3 space-y-2">
                    {attachedFiles.map((file, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm">
                        ...
                        <button onClick={() => removeFile(idx)} className="text-slate-400 hover:text-red-500 transition-colors"><X size={16} /></button>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {/* Modal Footer */}
            <div className="p-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              <button onClick={() => setIsEmailModalOpen(false)} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-50 font-bold text-sm transition-colors">Cancel</button>
              {/* handleSendEmail -> TODO: [BACKEND] ตรงนี้เรียก API ส่งอีเมล พร้อมแนบไฟล์ */}
              <button onClick={handleSendEmail} disabled={isLoading} className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-bold text-sm shadow-md shadow-indigo-200 flex items-center gap-2 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed transition-all">
                  {isLoading ? <Loader2 size={16} className="animate-spin"/> : <Send size={16}/>} Confirm Send
              </button>
            </div>
          </div>
        </div>
      ...
      {isSuccessModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center max-w-sm w-full animate-in zoom-in-95 duration-200">
                <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-4"><CheckCircle2 size={40} /></div>
                <h3 className="text-xl font-bold text-slate-800 mb-2">ส่งรายงานเรียบร้อย</h3>
                <p className="text-slate-500 text-center mb-6">ระบบได้ทำการส่งอีเมลรายงานประจำวันให้ผู้รับเรียบร้อยแล้ว</p>
                <button onClick={() => setIsSuccessModalOpen(false)} className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg shadow-indigo-200 transition-transform active:scale-95">ตกลง</button>
            </div>
        </div>
      )}
...
// --- Helper Components (Dashboard StatCard & StatusSummaryCard) ---
// Note: Copied from DailyReport logic to maintain visual consistency
